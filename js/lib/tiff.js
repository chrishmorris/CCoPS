...
TIFFParser.prototype = {
    ...
    getBits : function(numBits, byteOffset, bitOffset) {
        ...
        if (totalBits <= 0) {
            ...
            var rawBits = this.tiffDataView.getUint16(newByteOffset,
                    this.littleEndian);
        } else if (totalBits <= 32) {
            var shiftLeft = bitOffset;
            var rawBits = this.tiffDataView.getUint32(newByteOffset,
                    this.littleEndian);
        } else {
            console.log(numBits, byteOffset, bitOffset);
    ...
    getFieldValues : function(fieldTagName, fieldTypeName, typeCount,
            valueOffset) {
        var fieldValues = [];
            ...
            for ( var i = 0; i < typeCount; i++) {
                ...
                if (fieldTypeLength >= 8) {
                    if ([ 'RATIONAL', 'SRATIONAL' ].indexOf(fieldTypeName) !== -1) {
                        // Numerator
                        fieldValues.push(this.getBytes(4, valueOffset
                                + indexOffset));
                        // Denominator
                        fieldValues.push(this.getBytes(4, valueOffset
                                + indexOffset + 4));
    ...
    parseFileDirectory : function(byteOffset) {
        ...
        for ( var i = byteOffset + 2, entryCount = 0; entryCount < numDirEntries; i += 12, entryCount++) {
            ...
            var fieldValues = this.getFieldValues(fieldTagName, fieldTypeName,
                    typeCount, valueOffset);
    ...
    parseTIFF : function(tiffArrayBuffer, canvas, onerror) {
        ...
        var compression = fileDirectory.Compression ? fileDirectory.Compression.values[0]
                : 1;
        ...
        fileDirectory.BitsPerSample.values.forEach(function(bitsPerSample, i,
                bitsPerSampleValues) {
            sampleProperties[i] = {
        ...
        // StripByteCounts is supposed to be required, but see if we can recover
        // anyway.
        if (fileDirectory.StripByteCounts) {
            ...
            if (numStripOffsetValues === 1) {
                var stripByteCountValues = [ Math.ceil(imageWidth * imageLength
                        * bitsPerPixel / 8) ];
            } else {
        ...
        for ( var i = 0; i < numStripOffsetValues; i++) {
            ...
            for ( var byteOffset = 0, bitOffset = 0, jIncrement = 1, getHeader = true, pixel = [], numBytes = 0, sample = 0, currentSample = 0; byteOffset < stripByteCount; byteOffset += jIncrement) {
                ...
                switch (compression) {
                    ...
                    case 1:
                        ...
                        for ( var m = 0, pixel = []; m < samplesPerPixel; m++) {
                            if (sampleProperties[m].hasBytesPerSample) {
                                // XXX: This is wrong!
                                var sampleOffset = sampleProperties[m].bytesPerSample
                                        * m;
                                ...
                                var sampleInfo = this.getBits(
                                        ...
                                        stripOffset + byteOffset, bitOffset);

                                pixel.push(sampleInfo.bits);

                                byteOffset = sampleInfo.byteOffset
                                        - stripOffset;
                    ...
                    // CITT Group 3 1-Dimensional Modified Huffman run-length
                    // encoding
                    case 2:
                        // XXX: Use PDF.js code?
                        throw RangeError('Unsupported compression: '
                                + compression);
                        break;
                    ...
                    case 32773:
                        ...
                        if (getHeader) {
                            ...
                            var header = this.tiffDataView.getInt8(stripOffset
                                    + byteOffset, this.littleEndian);

                            if (header >= 0 && header <= 127) { // Normal
                                // pixels.
                                blockLength = header + 1;
                            } else if (header >= -127 && header <= -1) { // Collapsed
                                // pixels.
                                iterations = -header + 1;
                            } else /* if (header === -128) */{ // Placeholder
                                // byte?
                                getHeader = true;
                            }
                        } else {
                            var currentByte = this.getBytes(1, stripOffset
                                    + byteOffset);

                            // Duplicate bytes, if necessary.
                            for ( var m = 0; m < iterations; m++) {
                                if (sampleProperties[sample].hasBytesPerSample) {
                                    // We're reading one byte at a time, so we
                                    // need to handle multi-byte samples.
        ...
        if (fileDirectory.ColorMap) {
            ...
            var colorMapSampleSize = Math.pow(2,
        ...
        for ( var i = 0; i < numStrips; i++) {
            ...
            console.log("tiff rows: " + numRowsInStrip);
            for ( var y = 0, j = 0; y < numRowsInStrip, j < numPixels; y++) {
                ...
                for ( var x = 0; x < imageWidth; x++, j++) {
                    ...
                    switch (photometricInterpretation) {
                        ...
                        case 0:
                            if (sampleProperties[0].hasBytesPerSample) {
                                var invertValue = Math.pow(0x10,
                                        sampleProperties[0].bytesPerSample * 2);
                            }

                            // Invert samples.
                            pixelSamples.forEach(function(sample, index,
                                    samples) {
                            ...
                            // BlackIsZero
                        case 1:
                            red = green = blue = this.clampColorSample(
                                    pixelSamples[0],
                                    sampleProperties[0].bitsPerSample);
                            break;
                        ...
                        // RGB Full Color
                        case 2:
                            red = this.clampColorSample(pixelSamples[0],
                                    sampleProperties[0].bitsPerSample);
                            green = this.clampColorSample(pixelSamples[1],
                                    sampleProperties[1].bitsPerSample);
                            blue = this.clampColorSample(pixelSamples[2],
                        ...
                        case 3:
                            ...
                            red = this.clampColorSample(
                                    colorMapValues[colorMapIndex], 16);
                            green = this.clampColorSample(
                                    colorMapValues[colorMapSampleSize
                                            + colorMapIndex], 16);
                            blue = this.clampColorSample(colorMapValues[2
                                    * colorMapSampleSize + colorMapIndex], 16);
                            break;
                    ...
                    ctx.fillStyle = this.makeRGBAFillValue(red, green, blue,
                            opacity);
        ...
        /*
         * for (var i = 0, numFileDirectories = this.fileDirectories.length; i <
         * numFileDirectories; i++) { // Stuff }
         */
